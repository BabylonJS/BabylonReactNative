...
Additionally, there are currently a few manual steps required (work is planned to eliminate these):
1. From the Mac terminal (not PowerShell), starting at the React Native project root, run the following commands. This needs to be done on every developer/build machine after installing node modules.
```
cd node_modules/@babylonjs/react-native/ios
...
2. Add the generated ReactNativeBabylon.xcodeproj to your projects XCode Workspace (`.xcworkspace`) file. This can be done within XCode, and this change can be committed to source control so it only has to be done once.
