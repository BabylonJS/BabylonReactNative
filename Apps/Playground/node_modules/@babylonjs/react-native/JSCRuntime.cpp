// NOTE: This file was copied from the JSCRuntime.cpp included in React Native 0.62.1 with modifications in order to:
//   1. Access the underlying JSC JSGlobalContextRef.
//   2. Create a custom JSCRuntime with bug fixes and performance improvements.
...
std::string JSStringToSTLString(JSStringRef str) {
  ...
  result.resize(JSStringGetUTF8CString(str, result.data(), result.size()) - 1);
  return result;
}
...
jsi::Value JSCRuntime::createValue(JSValueRef value) const {
  auto type = JSValueGetType(ctx_, value);
  switch (type) {
