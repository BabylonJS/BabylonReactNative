...
namespace Babylon
{
    ...
    public:
        Impl(facebook::jsi::Runtime* jsiRuntime)
            : env{ Napi::Attach(reinterpret_cast<facebook::jsc::JSCRuntime*>(jsiRuntime)->getContext())}
        {
        }
...
//        }};
        ...
        JsRuntime::DispatchFunctionT dispatchFunction{[env = m_impl->env, jsRunLoop = std::move(jsRunLoop)](std::function<void(Napi::Env)> func) {
            CFRunLoopPerformBlock(jsRunLoop, kCFRunLoopDefaultMode, ^{
                func(env);
            });
        }};
        
        m_impl->runtime = &JsRuntime::CreateForJavaScript(m_impl->env, dispatchFunction);
