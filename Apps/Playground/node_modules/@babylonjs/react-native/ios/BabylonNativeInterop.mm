...
//static std::unique_ptr<std::unordered_map<jsi::Runtime*, Babylon::Native*>> nativeInstances2 = std::make_unique<std::unordered_map<jsi::Runtime*, Babylon::Native*>>();
static std::unordered_map<jsi::Runtime*, Babylon::Native*> nativeInstances2;
static std::unordered_map<jsi::Runtime*, std::vector<RCTPromiseResolveBlock>> initializationPromises;
static std::mutex mapMutex;
//static NSMutableDictionary* nativeInstances = [NSMutableDictionary dictionaryWithCapacity:1];
...
//    std::function<void(int)> doSomething = [](int val)
//    {
//        int y = val;
//    };
    
//    [nativeInstances setValue:@"foo" forUndefinedKey:@"bar"];

    [jsRunLoop performBlock:^{
        //[nativeInstances setValue:@"foo" forUndefinedKey:@"bar"];
        jsi::Runtime* jsiRuntime = GetJSIRuntime(bridge);
        //[nativeInstances setObject:@"hi" forKey:[NSValue valueWithPointer:jsiRuntime]];
        auto nativeInstancesIterator = nativeInstances2.find(jsiRuntime);
        if (nativeInstancesIterator == nativeInstances2.end()) {
...
+ (void)addNativeInstance:(jsi::Runtime*)jsiRuntime babylonNative:(Babylon::Native*)babylonNative {
    ...
    nativeInstances2[jsiRuntime] = babylonNative;
