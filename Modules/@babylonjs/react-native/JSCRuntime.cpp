// NOTE: This file was copied from the JSCRuntime.cpp included in React Native 0.63.1 with modifications in order to:
...
std::unique_ptr<jsi::Runtime> makeJSCRuntime(JSGlobalContextRef ctx) {
  return std::make_unique<JSCRuntime>(ctx);
}

JSGlobalContextRef getJSGlobalContextRefFromJSCRuntime(jsi::Runtime &runtime) {
  return reinterpret_cast<JSCRuntime&>(runtime).getContext();
}
